...
def count_cond(condition: Callable[[int, int], bool]) -> Callable[[int], int]:
    ...
    def inner(n):
        return sum(1 for i in range(1, n + 1) if condition(n, i))
...
def cycle(
    f1: Callable[[Any], Any], f2: Callable[[Any], Any], f3: Callable[[Any], Any]
) -> Callable[[Any], Any]:
    ...
    def times(n: int):
        def exe(x):
            for _, f in zip(range(n), itertools.cycle([f1, f2, f3])):
                x = f(x)
            return x
