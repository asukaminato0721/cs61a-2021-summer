...
def read(s: str) -> Union[CallExpr, LambdaExpr, Literal, Name, None]:
...
def take(src: Buffer, allowed_characters: Set[str]) -> Union[str, None]:
    result = ""
...
def next_token(src: Buffer) -> Union[str, None]:
...
def is_literal(s: Union[int, float]) -> bool:
...
## Parser ##
############
def read_expr(src: Buffer) -> Union[LambdaExpr, CallExpr]:
    token = src.pop_first()
    if token is None:
...
def read_call_expr(src: Buffer, operator) -> CallExpr:
    while src.current() == "(":
        src.pop_first()
