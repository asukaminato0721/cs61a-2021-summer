...
class AntsGUI:
    ...
    def _init_control_panel(self, gamestate):
        ...
        for name, ant_type in gamestate.ant_types.items():
            ...
            cost_pos = shift_point(
                panel_pos, (width / 2, ANT_IMAGE_HEIGHT + 4 + PANEL_PADDING[1])
            )
    ...
    def _init_places(self, gamestate):
        ...
        for name, place in gamestate.places.items():
            ...
            def on_click(gamestate, frame, name=name):
                ...
                if ant_type == "Remover":
                    ...
                    try:
                        print(
                            "gamestate.deploy_ant('{0}', '{1}')".format(name, ant_type)
                        )
            ...
            frame = self.add_click_rect(place_pos, width, height, on_click, color=color)
        ...
        self.place_points[gamestate.beehive.name] = (place_pos[0] + width, HIVE_HEIGHT)
    ...
    def strategy(self, gamestate):
        ...
        for ant in gamestate.ants + [
            a.ant_contained for a in gamestate.ants if has_ant(a)
        ]:
    ...
    def _update_places(self, gamestate):
        ...
        for name, place in gamestate.places.items():
            ...
            if place.ant is not None:
                if (
                    isinstance(place.ant, ants.ContainerAnt)
                    and place.ant.ant_contained
                    and place.ant.ant_contained not in current
                ):
            ...
            for insect in current - valid_insects:
                if (
                    not place.exit
                    or insect not in self.images[place.exit.name]
                    and insect not in place.entrance.bees
                ):
...
def animate_leaf(canvas, start, end, duration=0.3, color="ForestGreen"):
    ...
    length = 40
    leaf = canvas.draw_polygon(
        leaf_coords(start, 0, length), color="DarkGreen", fill_color=color, smooth=1
    )
...
@main
def run(*args):
    ants.Insect.reduce_health = class_method_wrapper(
        ants.Insect.reduce_health, pre=print_expired_insects
    )
