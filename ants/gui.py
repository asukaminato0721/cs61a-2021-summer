...
class GUI:
    ...
    def get_ant_types(self, noSave=False):
        ...
        for name, ant_type in self.gamestate.ant_types.items():
            ant_types.append(
                {
                    "name": name,
                    "cost": ant_type.food_cost,
                    "img": self.get_insect_img_file(name),
                }
            )
    ...
    def _init_places(self, gamestate):
        ...
        # Add the Hive
        self.places[gamestate.beehive.name] = {
            "name": name,
            "type": "beehive",
            "water": 0,
            "insects": {},
        ...
        for bee in gamestate.beehive.bees:
            self.places[gamestate.beehive.name]["insects"].append(
                {"id": self.currentBeeId, "type": "bee"}
            )
                ...
                if place.ant is not None:
                    ...
                    if ant_container and place.ant.ant_contained:
                        self.places[pRow][pCol]["insects"]["contains"] = {
                            ...
                            "img": self.get_insect_img_file(
                                place.ant.ant_contained.name
                            ),
                        }
            else:
                self.places[pRow][pCol]["insects"] = {}
...
def update():
    request = urllib.request.Request(
        "https://api.github.com/repos/colinschoen/Ants-Web-Viewer/releases/latest"
    )
    data = None
    ...
    if data:
        ...
        if release_version > VERSION:
            ...
            print(
                "Local version of",
                VERSION,
                "is current with or ahead of remote version of",
                release_version,
            )
