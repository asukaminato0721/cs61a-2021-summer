...
if sys.version_info[0] < 3:
    ...
    def surrogateescape_handler(exc):
        # Source: https://github.com/PythonCharmers/python-future/blob
        # /aef57391c0cd58bf840dff5e2bc2c8c0f5b0a1b4/src/future/utils
        # /surrogateescape.py
...
def sql_commands(read_line: Callable[[int, bool, str], str]):
    delims = ['"', "'", ";", "--"]
    counter = 0
    in_string: Optional[bool] = None
    j = i = 0
            ...
            if not in_string:
                if j < len(line):
                    ...
                    if delim == ";":
                        ...
                        concat.append(
                            line[j : j + len(delim)]
                        )  # ensure delimeter is the same type as the string
                        # (it may not be due to implicit conversion)
...
class WindowsConsoleIOMixin(object):
    ...
    # We use ReadConsole when we can, so it doesn't affect us, but it's good
    # info to know regardless.
    def __init__(self, fd):
        assert isatty(fd), (
            "file descriptor must refer to a console (note that on Windows, "
            "NUL satisfies isatty(), but is not a console)"
        )
        self.fd = fd
...
class WindowsConsoleTextIO(WindowsConsoleIOMixin, io.TextIOBase):
    ...
    @staticmethod  # Don't let classes override this... they can override the
    # caller instead
    def do_read(self, nchars, translate_newlines):
        ...
        while True:
            ...
            if chunk.startswith(
                "\x1A"
            ):  # EOF on Windows (Ctrl+Z) at the beginning of a line results
                # in the entire rest of the buffer being discarded
                iend = istart
                break
            # Python 2 and Python 3 behaviors differ on Windows... Python 2's
            # sys.stdin.readline() just deletes the next character if it sees
            # EOF in the middle of a string! I won't emulate that here.
            self.buffered += chunk  # We're relying on Python's concatenation
            # optimization here... we don't do it ourselves, since we want
            # self.buffered to be valid every iteration in case there is an
            # exception raised
...
class NonOwningTextIOWrapper(io.TextIOWrapper):
    def __init__(self, base_textiowrapper, **kwargs):
        ...
        self.base = base_textiowrapper  # must keep a reference to this alive
...
):  # The underlying stream must NOT be used directly until the stream
    # returned by this function is disposed of
    if isinstance(stream, io.TextIOWrapper):
        stream.flush()  # Make sure nothing is left in the buffer before we
        # re-wrap it
        none = object()
        kwargs = {}
        for key in [
        ...
        for arg in args:
            if first:
                ...
                StdIOProxy._do_write(
                    stream,
                    allow_set_code_page,
                    self._coerce(streaminfo, codec, sep),
                )
            StdIOProxy._do_write(
                stream,
                allow_set_code_page,
                self._coerce(streaminfo, codec, arg),
            )
        if end is not None:
            StdIOProxy._do_write(
                stream,
                allow_set_code_page,
                self._coerce(streaminfo, codec, end),
            )
        if flush:
            stream.flush()
...
class bytes_comparable_with_unicode(
    bytes
):  # For Python 2/3 compatibility, to allow implicit conversion between
    # strings and bytes when it is safe. (Used for strings like literals
    # which we know be safe.)
    codec = codecs.lookup("ascii")  # MUST be a safe encoding

    @classmethod
    def coerce(cls, other, for_output: bool = False):
        return (
...
def main(
    program, *args, **kwargs
):  # **kwargs = dict(stdin=file, stdout=file, stderr=file); useful for
    # callers who import this module
    import argparse  # slow import (compiles regexes etc.), so don't import it
    ...
    argparser.add_argument(
        ...
        default="true",
        choices=("true", "false"),
        help="whether to enable Unicode wrapper for standard I/O (default: %("
        "default)s)",
    )
    argparser.add_argument(
        "--console",
        ...
        default="true",
        choices=("true", "false"),
        help="whether to auto-detect and use console window APIs (default: %("
        "default)s)",
    )
    argparser.add_argument(
        "--encoding",
    ...
    )  # This is only necessary on Python 2 if we use the default I/O
    # functions instead of bypassing to ReadConsole()/WriteConsole()
    ...
    def exec_script(db, filename, ignore_io_errors):
        try:
            ...
            ) as f:  # Assume .sql files are text -- any binary data inside
                # them should be X'' encoded, not embedded directly
            ...
            if index < len(init_sql):
                ...
                if show_prompt:
                    if not_in_the_middle_of_any_input:
                        ...
                        if index == 0:
                            to_write.append(
                                "SQLite version %s (adapter version "
