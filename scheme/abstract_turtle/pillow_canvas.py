...
class PillowCanvas(Canvas):
    ...
    def draw_rectangular_line(self, start, end, color, width):
        self.draw.line(
            [self.tr_pos(start), self.tr_pos(end)], self.tr_color(color), width
        )
    ...
    def fill_path(self, path, color):
        ...
        self.draw.polygon(
            [self.tr_pos(point) for point in points], fill=self.tr_color(color)
        )
    ...
    def axis_aligned_rectangle(self, bottom_left, width, height, color):
        ...
        print(bounding_box)
        self.draw.rectangle(bounding_box, fill=self.tr_color(color))
