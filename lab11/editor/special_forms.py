...
from datamodel import Expression, Symbol, Pair, SingletonTrue, SingletonFalse, \
    Nil, Undefined, Promise, NilType, String
...
from helper import pair_to_list, verify_exact_callable_length, \
...
from scheme_exceptions import OperandDeduceError, IrreversibleOperationError, \
    LoadError, SchemeError, TypeMismatchError, \
...
class ProcedureObject(Callable):
    ...
    def execute(self, operands: List[Expression], frame: Frame,
                gui_holder: Holder, eval_operands=True):
        ...
        gui_holder.expression.set_entries(
            [VisualExpression(expr, gui_holder.expression.display_value) for
             expr in body])
    ...
    def __repr__(self):
        if self.var_param is not None:
            ...
            varparams = ""
        return f"({self.name} {' '.join(map(repr, self.params))}{varparams}) " \
               f"[parent = {self.frame.id}]"
...
class ProcedureBuilder(Callable):
    ...
    def execute(self, operands: List[Expression], frame: Frame,
                gui_holder: Holder, name: str = "lambda"):
        ...
        for i, param in enumerate(params):
            if (logger.dotted or i != len(params) - 1) and not isinstance(param,
                                                                          Symbol):
                raise OperandDeduceError(
            ...
            if isinstance(param, Pair):
                ...
                if len(param_vals) != 2 or \
                    ...
                    raise OperandDeduceError(
                        f"Each member of a parameter list must be a Symbol or "
                        f"a variadic "
...
class DefineMacro(Callable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        if not isinstance(params, Pair):
            raise OperandDeduceError(
                f"Expected a Pair, not {params}, as the first operand of "
                f"define-macro.")
        name = params.first
        ...
        frame.assign(name,
                     Macro().execute(operands, frame, gui_holder, name.value))
        return name
...
class Define(Callable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        if isinstance(params, Symbol):
            ...
            frame.assign(params, evaluate(operands[1], frame,
...
class Set(Callable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        frame.mutate(name, evaluate(operands[1], frame,
                                    gui_holder.expression.children[2]))
        return Undefined
...
class If(Callable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        if evaluate(operands[0], frame,
                    gui_holder.expression.children[1]) is SingletonFalse:
            if len(operands) == 2:
                return Undefined
            else:
                return evaluate(operands[2], frame,
                                gui_holder.expression.children[3], True)
        else:
            return evaluate(operands[1], frame,
                            gui_holder.expression.children[2], True)
...
class Eval(Applicable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        verify_exact_callable_length(self, 1, len(operands))
        if eval_operands:
            operand = evaluate(operands[0], frame,
                               gui_holder.expression.children[1])
        else:
            operand = operands[0]
        gui_holder.expression.set_entries(
            [VisualExpression(operand, gui_holder.expression.display_value)])
        gui_holder.apply()
...
class Apply(Applicable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        gui_holder.expression.set_entries([VisualExpression(Pair(func, args),
                                                            gui_holder.expression.display_value)])
        gui_holder.expression.children[0].expression.children = []
        gui_holder.apply()
        args = pair_to_list(args)
        return func.execute(args, frame, gui_holder.expression.children[0],
                            False)
...
class Let(Callable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        for i, binding in enumerate(bindings):
            ...
            if not isinstance(name, Symbol):
                raise OperandDeduceError(
                    f"Expected first element of binding to be a Symbol, "
                    f"not {name}.")
            new_frame.assign(name, evaluate(expr, frame,
                                            binding_holder.expression.children[
                                                1]))
...
class Quasiquote(Callable):
    def execute(self, operands: List[Expression], frame: Frame,
                gui_holder: Holder):
        verify_exact_callable_length(self, 1, len(operands))
        return Quasiquote.quasiquote_evaluate(operands[0], frame,
                                              gui_holder.expression.children[1])

    @classmethod
    def quasiquote_evaluate(cls, expr: Expression, frame: Frame,
                            gui_holder: Holder, splicing=False):
        ...
        if isinstance(expr, Pair):
            try:
                ...
                is_well_formed = not any(map(
                    lambda x: isinstance(x, Symbol) and x.value in ["unquote",
                                                                    "quasiquote",
                                                                    "unquote-splicing"],
                    lst))
        ...
        if isinstance(expr, Pair):
            if isinstance(expr.first, Symbol) and expr.first.value in (
            "unquote", "unquote-splicing"):
                ...
                if is_well_formed:
                    ...
                    for sub_expr, holder in zip(pair_to_list(expr),
                        ...
                        splicing = isinstance(sub_expr, Pair) and isinstance(
                            sub_expr.first, Symbol) \
                                   and sub_expr.first.value == \
                                   "unquote-splicing"
                        evaluated = Quasiquote.quasiquote_evaluate(sub_expr,
                                                                   frame,
                                                                   holder,
                                                                   splicing)
                        if splicing:
                    ...
                    out = Pair(Quasiquote.quasiquote_evaluate(expr.first, frame,
                                                              visual_expression.children[
                                                                  0]),
                               Quasiquote.quasiquote_evaluate(expr.rest, frame,
                                                              visual_expression.children[
                                                                  1]))
                visual_expression.value = out
                gui_holder.complete()
    ...
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        try:
            with open(f"{operands[0].value}.scm") as file:
                ...
                gui_holder.expression.set_entries([VisualExpression(expr,
                                                                    gui_holder.expression.display_value)])
                gui_holder.apply()
...
class LoadAll(Applicable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        try:
            ...
            targets = [join(directory, target) for target in targets if
                       target.endswith(".scm")]
            exprs = [make_list(
                [Symbol("load"), make_list([Symbol("quote"), Symbol(x[:-4])])])
                     for x in targets]
            equiv = make_list([Symbol("begin-noexcept")] + exprs)
            gui_holder.expression.set_entries([equiv])
            gui_holder.apply()
            return evaluate(equiv, frame, gui_holder.expression.children[0],
                            True)
...
class BeginNoExcept(Callable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        for i, (operand, holder) in enumerate(
            ...
            SchemeError, RecursionError, ValueError, ZeroDivisionError) as e:
...
class Force(Applicable):
    def execute(self, operands: List[Expression], frame: Frame,
        ...
        if eval_operands:
            operand = \
            evaluate_all(operands, frame, gui_holder.expression.children[1:])[0]
        ...
        if logger.fragile:
            raise IrreversibleOperationError()
        gui_holder.expression.set_entries([VisualExpression(operand.expr,
                                                            gui_holder.expression.display_value)])
        gui_holder.apply()
