...
class ProcedureObject(Callable):
    ...
    def __repr__(self):
        if self.var_param is not None:
            ...
            varparams = ""
        return (
            f"({self.name} {' '.join(map(repr, self.params))}{varparams}) "
...
class ProcedureBuilder(Callable):
    ...
    def execute(
        ...
        for i, param in enumerate(params):
            if (logger.dotted or i != len(params) - 1) and not isinstance(
                param, Symbol
            ):
                raise OperandDeduceError(
...
class Define(Callable):
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        if isinstance(params, Symbol):
            ...
            frame.assign(
                params, evaluate(operands[1], frame, gui_holder.expression.children[2])
            )
            return params
            ...
            if not isinstance(name, Symbol):
                raise OperandDeduceError(f"Expected a Symbol, not {name}.")
            frame.assign(
                name, Lambda().execute(operands, frame, gui_holder, name.value)
            )
            return name
        else:
            raise OperandDeduceError(
...
class Set(Callable):
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        frame.mutate(
            name, evaluate(operands[1], frame, gui_holder.expression.children[2])
        )
        return Undefined
...
class If(Callable):
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        if (
            evaluate(operands[0], frame, gui_holder.expression.children[1])
            is SingletonFalse
        ):
            if len(operands) == 2:
                return Undefined
    ...
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        for cond_i, cond in enumerate(operands):
            if not isinstance(cond, Pair):
                raise OperandDeduceError(
                    f"Unable to evaluate clause of cond, as {cond} is not a " f"Pair."
                )
            ...
            if (
                isinstance(expanded[0], Symbol) and expanded[0].value == "else"
...
class Let(Callable):
    def execute(self, operands: List[Expression], frame: Frame, gui_holder: Holder):
        ...
        for i, binding in enumerate(bindings):
            ...
            if not isinstance(name, Symbol):
                raise OperandDeduceError(
                    f"Expected first element of binding to be a Symbol, " f"not {name}."
                )
            new_frame.assign(
                name, evaluate(expr, frame, binding_holder.expression.children[1])
            )
...
class Quasiquote(Callable):
    ...
    @classmethod
    def quasiquote_evaluate(
        cls, expr: Expression, frame: Frame, gui_holder: Holder, splicing=False
    ):
        ...
        if isinstance(expr, Pair):
            if isinstance(expr.first, Symbol) and expr.first.value in (
                ...
                if is_well_formed:
                    out = []
                    for sub_expr, holder in zip(
                        pair_to_list(expr), visual_expression.children
                    ):
                        splicing = (
                            isinstance(sub_expr, Pair)
